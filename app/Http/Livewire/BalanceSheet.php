...
{ public $datefrom;
    ...
    {
        try {
            return redirect()->route('Balance-Sheet', ['slug' => $this->datefrom, 'slug2' => $this->dateto]);
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'An error occurred: ' . $e->getMessage());
        }
    }
    ...
    {
        try {
            return redirect()->route('Balance-Sheet', ['slug' => $this->datefrom, 'slug2' => $this->dateto, 'slug3' => $this->branchto]);
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'An error occurred: ' . $e->getMessage());
        }
    }
    public function generateBalanceSheet()
    {
        ...
        $pdf = Pdf::loadView('Balance-Sheet', $data);
        $pdf->setPaper('A4', 'potrait');
        $pdfName =  'Balance-Sheet'.$this->slug2 . $this->slug . $this->slug3 . '.pdf';
        $filePath = 'documents/balancesheet/' . $pdfName;
