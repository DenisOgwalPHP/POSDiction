    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>Damages Records</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="/Dashboard">Home</a></li>
                            <li class="breadcrumb-item active">Damages Records</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="card card-info" wire:ignore>
                            <div class="card-header">
                                <h3 class="card-title">Damages Records Listing</h3>
                            </div>
                            <div class="card-body">
                                    ...
                                    <tbody>
                                        @foreach ($registereddamages as $registereddamage)
                                            <tr>
                                                ...
                                                <td>{{ $registereddamage->id }}</td>
                                                <td>{{ $registereddamage->damagedproduct->ProductName }}</td>
                                                <td>{{ $registereddamage->damagedproduct->Weight }}</td>
                                                <td>{{ $registereddamage->damagedproduct->Origin }}</td>
                                                <td>{{ $registereddamage->Quantity }}</td>
                                                <td>{{ $registereddamage->PurchaseRefer }}</td>
                                                <td>{{ $registereddamage->damagedforsupplier->AccountName }}</td>
                                                @if ($registereddamage->Branch == 100)
                                                    <td>Central Store</td>
                                                @else
                                                    <td>{{ $registereddamage->damagebranch->BranchName }}</td>
                                                @endif
                                                <td>{{ $registereddamage->registrar->name }}</td>
                                                <td>{{ $registereddamage->created_at }}</td>
                                                <td>{!! $registereddamage->Reason !!}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
